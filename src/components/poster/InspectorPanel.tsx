...
const G = () => isDarkTheme() ? {
  ...
  goldFaint: "rgba(255,255,255,0.07)",
  goldBorder: "rgba(255,255,255,0.18)",
  divider: "rgba(255,255,255,0.14)",
} : {
  gold: "#111111",
  goldBright: "#111111",
  goldDim: "rgba(0,0,0,0.55)",
  goldFaint: "rgba(0,0,0,0.05)",
  goldBorder: "rgba(0,0,0,0.15)",
  divider: "rgba(0,0,0,0.1)",
};

const textColor = () => isDarkTheme() ? "#ffffff" : "#111111";
const textColorDim = () => isDarkTheme() ? "rgba(255,255,255,0.75)" : "#444444";
const inputBg = () => isDarkTheme() ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.04)";
    ...
    >
      ...
      >
        <div className="h-5 w-5 rounded flex items-center justify-center flex-shrink-0"
          style={{ background: open ? g.goldFaint : "transparent", border: `1px solid ${g.goldBorder}` }}
        >
          <Icon className="h-3 w-3" style={{ color: g.gold }} />
        </div>
        <span className="flex-1 text-left text-[9px] uppercase"
          style={{ fontFamily: "'Cinzel', serif", letterSpacing: "0.2em", color: g.goldBright }}>
          {title}
        </span>
        {badge && (
...
const MiniButton = ({ active, onClick, children, title }: { active?: boolean; onClick: () => void; children: React.ReactNode; title?: string }) => (
  <button onClick={onClick} title={title}
    ...
    style={active ? {
      background: isDarkTheme() ? `#ffffff` : `#111111`,
      color: isDarkTheme() ? "#111111" : "#ffffff",
      boxShadow: isDarkTheme() ? `0 2px 8px rgba(255,255,255,0.15)` : `0 2px 8px rgba(0,0,0,0.25)`,
...
const InspectorPanel = ({
  ...
  useEffect(() => { const observer = new MutationObserver(() => setIsDark(document.documentElement.classList.contains("dark"))); observer.observe(document.documentElement, { attributeFilter: ["class"] }); return () => observer.disconnect(); }, []);
  ...
  return (
    <div
      ...
      style={{ ...panelStyle, height: "100%", overflow: "hidden" }}
    >
      {/* ── Luxury Header ── */}
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <span style={{ fontFamily: "'Cinzel', serif", fontSize: "9px", letterSpacing: "0.45em", color: G().goldDim }}>STUDIO</span>
      ...
      >
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon className="h-3.5 w-3.5" style={{ color: G().goldBright }} />
              <span style={{
                fontFamily: "'Cinzel', serif",
                fontSize: "8px",
                letterSpacing: "0.2em",
      ...
      {/* ── Scrollable Content ── */}
      <div className="flex-1 min-h-0 overflow-y-auto inspector-scroll" style={{ scrollbarWidth: "thin", scrollbarColor: isDark ? "rgba(255,255,255,0.3) transparent" : "rgba(0,0,0,0.25) transparent" }}>
      ...
      <Section title="Background Gallery" icon={Palette} defaultOpen={false} badge={`${BG_GALLERY.length}`}>
        <div className="grid grid-cols-3 gap-1.5">
          {BG_GALLERY.map((bg) => (
            <button key={bg.label} onClick={() => onUpdatePoster({ backgroundImage: null, backgroundColor: "#000", overlayGradient: bg.css })} className="relative h-14 rounded-lg overflow-hidden hover:ring-2 hover:ring-accent transition-all group" style={{ background: bg.css }} title={bg.label}>
      ...
      <Section title="Adjustments" icon={SunMedium} defaultOpen={false} badge="NEW">
        ...
        <div>
          ...
          <span className="text-[9px] text-muted-foreground/50 font-mono">{poster.hueRotate ?? 0}°</span>
        </div>
        <div>
        ...
        {activeTab === "element" && (
          <div>
                ...
                {FONT_STYLE_PRESETS.map((preset) => (
                  <button
                    ...
                    style={selectedElement.fontFamily === preset.family ? {
                      background: isDarkTheme() ? `#ffffff` : `#111111`,
                      color: isDarkTheme() ? "#111111" : "#ffffff",
                      fontFamily: preset.family, fontWeight: 700,
                    } : {
            ...
            <Section title="Flip & Mirror" icon={Maximize2} defaultOpen={false}>
              <div className="grid grid-cols-2 gap-2">
                <MiniButton active={!!selectedElement.flipX} onClick={() => onUpdateElement(selectedElement.id, { flipX: !selectedElement.flipX })}>↔ Flip H</MiniButton>
                <MiniButton active={!!selectedElement.flipY} onClick={() => onUpdateElement(selectedElement.id, { flipY: !selectedElement.flipY })}>↕ Flip V</MiniButton>
              </div>
            </Section>
            ...
            <Section title="Transform" icon={Move}>
              ...
              <div>
                ...
                <span className="text-[9px] text-muted-foreground/50 font-mono">{selectedElement.rotation}°</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label>Skew X</Label>
                  ...
                  />
                  <span className="text-[9px] text-muted-foreground/50 font-mono">{selectedElement.skewX ?? 0}°</span>
                </div>
                <div>
                  <Label>Skew Y</Label>
                  <Slider
                    ...
                    min={-30} max={30} step={1} className="mt-1"
                  />
                  <span className="text-[9px] text-muted-foreground/50 font-mono">{selectedElement.skewY ?? 0}°</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
